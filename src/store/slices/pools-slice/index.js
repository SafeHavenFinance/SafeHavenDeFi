...
export const fetchPoolsPublicDataAsync = createAsyncThunk(
  ...
  async (pids) => {
    ...
    const sltStakes = stakesWithoutHelperLps.filter((stake) => stake.pid === 9);
   //const sltStakes = farmsToFetch.filter((stake) => stake.pid === 0);
    ...
    const stakesWithApr = stakesWithoutHelperLps.map((stake) => {
      ...
      let apr = getPoolApr(
        ...
        new BigNumber(stake.poolWeight)
      );
        
      //console.log("afasdfdas = ",stakes )
